...
public class CopyFilesModify implements IModify {
    ...
    public CopyFilesModify(String aName
            , IFile[] aFiles
            , String aDescription
            , String aJarDir
    ) {
    ...
    public CanApplyResult canApply(IMainModel aMainModel) {
        ...
        try {
            for (IFile file : theFiles) {
                ...
                if (in == null)
                    throw new ErrorApplyException("Файл " + fullJarName + " не найден во внутренних ресурсах программы");
                try {
                    File destFile = aMainModel.getFile(file) ;
                    if(destFile.exists()) {
                        BufferedInputStream jbossIn = new BufferedInputStream(
    ...
    protected static boolean isInputStreamEquals(InputStream aSource, InputStream aDest) throws IOException {
        ...
        while ( (sourceReaded=aSource.read()) >= 0
                && (destReaded=aDest.read()) >= 0) {
        ...
        try {
            for (IFile file : theFiles) {
                ...
                try {
                    ...
                    BufferedOutputStream jbossIn = new BufferedOutputStream(
